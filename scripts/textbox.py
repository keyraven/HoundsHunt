...
class TextBox(LayeredSprite):
    ...
    def __init__(self, rect, text:str, text_renderer:pygame.Font, groups = (), layer = 0, antialias:bool = True,
                 ...
                 animate = False, animate_speed:int = 12):
    ...
    def update_theme(self, new_theme:dict):
        if type(new_theme) is not dict: 
            new_theme = {}

        self.y_padding = new_theme.get("y_padding", self.theme_defaults["y_padding"])
        self.x_padding = new_theme.get("x_padding", self.theme_defaults["x_padding"])
        self.text_color = new_theme.get("text_color", self.theme_defaults["text_color"])
    ...
    def drawText(text:str, color, rect:pygame.Rect, font:pygame.Font, ypadding = 0, xpadding = 0, aa = False):
        ...
        while text:
            ...
            while font.size(text[:i])[0] < rect.width + 2*xpadding and i < len(text):
                i += 1
